...
export async function createTokenAwareChunks(doc: any, maxTokensPerChunk: number): Promise<any[]> {
  const chunks = [];
...
async function createTableAwareBlockChunks(text: string, blockIndex: number, maxTokensPerChunk: number): Promise<any[]> {
  const chunks = [];
