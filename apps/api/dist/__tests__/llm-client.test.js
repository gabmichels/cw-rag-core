import { LLMClientImpl, LLMClientFactoryImpl, ResilientLLMClientFactory, createLLMClientFactory } from '../services/llm-client.js';
...
describe('LLMClient', () => {
    ...
    describe('LLMClientImpl', () => {
        ...
        describe('vLLM provider', () => {
            ...
            it('should generate completion with vLLM', async () => {
                ...
                const client = new LLMClientImpl(config);
                const result = await client.generateCompletion('What is AI?', 'Context: AI is artificial intelligence');
            ...
            it('should generate streaming completion with vLLM', async () => {
                ...
                const client = new LLMClientImpl(config);
                const chunks = [];
                for await (const chunk of client.generateStreamingCompletion('What is AI?', 'Context: AI is artificial intelligence')) {
                    chunks.push(chunk);
                }
            ...
            it('should handle vLLM API errors', async () => {
                ...
                const client = new LLMClientImpl(config);
                await expect(client.generateCompletion('What is AI?', 'Context: AI is artificial intelligence')).rejects.toThrow('vLLM API error: 500 Internal Server Error');
            });
            it('should handle vLLM timeout', async () => {
                ...
                const client = new LLMClientImpl(config);
                await expect(client.generateCompletion('What is AI?', 'Context: AI is artificial intelligence')).rejects.toThrow('Request timeout');
            });
        });
        describe('Error handling', () => {
            ...
            it('should throw error for streaming with unsupported provider', async () => {
                ...
                const generator = client.generateStreamingCompletion('What is AI?', 'Context: AI is artificial intelligence');
    ...
    describe('Environment configuration', () => {
        ...
        it('should default to OpenAI when not configured', () => {
            ...
            const factory = new LLMClientFactoryImpl();
            const config = factory.getDefaultTenantConfig();
            expect(config.defaultConfig.provider).toBe('openai');
        });
    });
    describe('Token estimation', () => {
        it('should estimate tokens correctly', () => {
            ...
            const client = new LLMClientImpl(config);
            const tokens = client.estimateTokens('This is a test message');
