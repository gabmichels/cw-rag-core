...
export async function documentFetchRoute(fastify, options) {
        ...
        handler: async (request, reply) => {
            try {
                ...
                for (const point of scrollResult.points) {
                    if (!point.payload?.docId)
                        continue;
                    const docId = point.payload.docId;
                    const space = (point.payload.space || point.payload.spaceId || 'general');
                    const createdAtRaw = point.payload.createdAt;
                    const modifiedAtRaw = point.payload.modifiedAt;
                ...
                const documents = Array.from(documentsMap.values()).map(doc => {
                    ...
                    let category, badge, humanReadable;
                    if (ageInDays <= 7) {
                        category = 'Fresh';
                        badge = '🟢 Fresh';
                    }
        ...
        handler: async (request, reply) => {
            ...
            try {
                ...
                const sortedChunks = scrollResult.points
                    .filter((point) => point.payload?.content !== undefined && point.id !== undefined) // Filter and type assertion for payload (ensures id and payload are present)
                    .sort((a, b) => {
                ...
                const firstChunkPayload = sortedChunks[0].payload; // Cast to our defined ChunkPayload
                const documentMetadata = {
                    ...
                    docId: firstChunkPayload.docId || docId,
                    tenantId: firstChunkPayload.tenantId || 'unknown',
                    acl: Array.isArray(firstChunkPayload.acl) ? firstChunkPayload.acl : [],
                    authors: Array.isArray(firstChunkPayload.authors) ? firstChunkPayload.authors : undefined,
                    keywords: Array.isArray(firstChunkPayload.keywords) ? firstChunkPayload.keywords : undefined,
                    // Ensure optional string properties are indeed string | undefined
                    version: firstChunkPayload.version,
                    url: firstChunkPayload.url,
                    filepath: firstChunkPayload.filepath,
                    lang: firstChunkPayload.lang,
                    createdAt: firstChunkPayload.createdAt,
                    modifiedAt: firstChunkPayload.modifiedAt,
                };
