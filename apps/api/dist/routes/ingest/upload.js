...
export async function uploadRoute(fastify, options) {
    ...
    fastify.post('/upload', {
        ...
        handler: async (request, reply) => {
            ...
            try {
                ...
                const tenant = request.body?.tenant || 'default';
                const source = request.body?.source || 'upload';
                const acl = request.body?.acl ? JSON.parse(request.body.acl) : ['public'];
                const publish = request.body?.publish === 'true';
                const title = request.body?.title;
                const tags = request.body?.tags ? JSON.parse(request.body.tags) : [];
                const authors = request.body?.authors ? JSON.parse(request.body.authors) : [];
                const processed = [];
                const summary = {
                    total: files.length,
                    converted: 0,
                ...
                for (const file of files) {
                    try {
                        ...
                        const normalizedDoc = await convertFileToNormalizedDoc(file, tenant, source, acl, title, tags, authors);
                        summary.converted++;
                        if (publish) {
...
async function convertFileToNormalizedDoc(file, tenant, source, acl, title, tags, authors) {
    const extension = getFileExtension(file.filename);
...
function generateDocId(filename, tenant) {
