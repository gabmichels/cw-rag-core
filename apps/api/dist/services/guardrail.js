export class SimpleGuardrailService {
    ...
    evaluateAnswerability(query, searchResults, userContext) {
    ...
    calculateConfidence(mean, max, min, count) {
    ...
    generateIdkResponse(score, query) {
    ...
    generateReasoning(mean, max, min, count) {
        const issues = [];
        if (count === 0)
            issues.push('no results found');
        if (mean < 0.3)
            issues.push('low average relevance');
        if (max < 0.5)
            issues.push('low maximum relevance');
        if (count < 2)
            issues.push('insufficient result diversity');
