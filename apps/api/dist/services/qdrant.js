...
import { generateOptimizedCollectionConfig, calculateAdaptiveEf, QueryResultCache, QdrantPerformanceMonitor } from '@cw-rag-core/retrieval';
...
export async function searchDocuments(qdrantClient, collectionName, request, userContext, vector) {
 ...
 */
export async function searchDocumentsLegacy(qdrantClient, collectionName, request, userTenants, userAcl) {
...
export async function keywordSearchDocuments(qdrantClient, collectionName, query, limit, userContext) {
    // Validate user authorization first
    if (!validateUserAuthorization(userContext)) {
        throw new Error('User authorization validation failed');
    }
 ...
 */
export async function keywordSearchDocumentsLegacy(qdrantClient, collectionName, query, limit, userTenants, userAcl) {
    // Convert legacy parameters to UserContext for internal use
    const userContext = {
        id: userAcl[0] || '', // Assume first ACL entry is user ID
        groupIds: userAcl.slice(1), // Rest are group IDs
 ...
 */
export async function securityTestSearch(qdrantClient, collectionName, userContext, vector, limit = 10) {
 ...
 */
export async function batchSearchDocuments(qdrantClient, collectionName, userContext, vectors, limit = 5) {
    ...
    const searchPromises = vectors.map(vector => qdrantClient.search(collectionName, {
        vector: vector,
        limit: limit,
        filter: rbacFilter,
        with_payload: true,
    ...
    try {
        ...
        return searchResults.map(result => result.map((hit) => ({
            id: hit.id,
            vector: hit.vector || [],
            payload: hit.payload || {}
        })));
 ...
 */
export async function optimizedSearchDocuments(qdrantClient, collectionName, userContext, vector, limit = 5, exactSearch = false) {
    // Validate user authorization first
    if (!validateUserAuthorization(userContext)) {
        throw new Error('User authorization validation failed');
    }
