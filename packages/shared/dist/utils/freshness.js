...
const tenantFreshnessConfigs = new Map();
/**
 * Set custom freshness thresholds for a specific tenant
 */
export function setTenantFreshnessConfig(tenantId, config) {
    tenantFreshnessConfigs.set(tenantId, config);
}
/**
 * Get freshness thresholds for a specific tenant
 */
export function getTenantFreshnessThresholds(tenantId) {
 ...
 */
export function calculateDocumentAge(modifiedAt, currentTime) {
 ...
 */
export function categorizeFreshness(ageInDays, thresholds = DEFAULT_FRESHNESS_THRESHOLDS) {
 ...
 */
export function generateFreshnessBadge(category) {
    switch (category) {
        case 'Fresh':
            return '🟢 Fresh';
        case 'Recent':
 ...
 */
export function calculateFreshnessInfo(modifiedAt, tenantId, currentTime, createdAt) {
...
export function calculateFreshnessStats(documents, tenantId, currentTime) {
 ...
 */
export function calculateFreshnessInfoSafe(modifiedAt, tenantId, currentTime, createdAt) {
    try {
        const timestamp = modifiedAt || createdAt;
        if (!timestamp) {
            return null;
