import { calculateDocumentAge, categorizeFreshness, generateFreshnessBadge, formatHumanReadableAge, calculateFreshnessInfo, calculateFreshnessStats, setTenantFreshnessConfig, getTenantFreshnessThresholds, DEFAULT_FRESHNESS_THRESHOLDS } from '../utils/freshness.js';
describe('Freshness Utilities', () => {
    ...
    describe('categorizeFreshness', () => {
        const thresholds = { freshDays: 7, recentDays: 30 };
    ...
    describe('calculateFreshnessInfo', () => {
        ...
        it('should use createdAt as fallback', () => {
            ...
            const info = calculateFreshnessInfo(undefined, undefined, testTime, createdAt);
            expect(info.ageInDays).toBe(10);
            expect(info.timestamp).toBe(createdAt);
        });
        it('should throw error when no timestamp provided', () => {
            expect(() => calculateFreshnessInfo(undefined, undefined, testTime)).toThrow();
        });
        it('should use tenant-specific thresholds', () => {
            const tenantThresholds = { freshDays: 3, recentDays: 10 };
    ...
    describe('tenant configuration', () => {
        it('should set and get tenant-specific thresholds', () => {
            const customThresholds = { freshDays: 14, recentDays: 60 };
