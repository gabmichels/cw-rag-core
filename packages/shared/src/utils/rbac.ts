...
const effectiveGroupsCache = new Map<string, {
  groupIds: string[];
  timestamp: number;
  ttl: number;
}>();
...
const authValidationCache = new Map<string, {
  isValid: boolean;
  timestamp: number;
  ttl: number;
}>();
 ...
 */
function createEffectiveGroupsCacheKey(userContext: UserContext): string {
 ...
 * @returns Cache key string
 */
function createAuthValidationCacheKey(userContext: UserContext): string {
  return JSON.stringify({
    userId: userContext.id,
    groupIds: userContext.groupIds.sort(),
 ...
 */
function cleanupCache<T>(cache: Map<string, T & { timestamp: number; ttl: number }>): void {
  const now = Date.now();
 ...
 */
export function getEffectiveGroupIds(userContext: UserContext): string[] {
  // Check cache first
  ...
  if (userContext.groupHierarchy) {
    ...
    const addInheritedGroups = (groupId: string, visited = new Set<string>()) => {
 ...
 */
export function calculateLanguageRelevance(userLanguage?: string, docLanguage?: string): number {
 ...
 */
export function validateUserAuthorization(userContext: UserContext): boolean {
  // Check cache first
 ...
 */
export function clearRBACCaches(): void {
 ...
 */
export function filterDocumentsByAccess<T extends { metadata: DocumentMetadata; score?: number }>(
  userContext: UserContext,
  documents: T[]
): T[] {
  return documents
 ...
 */
export function getUserACLEntries(userContext: UserContext): string[] {
  const effectiveGroupIds = getEffectiveGroupIds(userContext);
