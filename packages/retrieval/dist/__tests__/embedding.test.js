...
describe('BgeSmallEnV15EmbeddingService', () => {
    ...
    it('should log structured error and throw after max retries', async () => {
        ...
        expect(console.error).toHaveBeenCalledWith('StructuredLog:EmbeddingServiceFailure', expect.objectContaining({
            texts: [mockText],
    ...
    it('should handle embedDocument by calling embed with document content', async () => {
        ...
        expect(mockedAxios.post).toHaveBeenCalledWith(expect.any(String), { texts: [mockDocument.content] }, expect.any(Object));
        expect(result).toHaveLength(EMBEDDING_DIMENSIONS);
        const magnitude = Math.sqrt(result.reduce((sum, val) => sum + val * val, 0));
        expect(magnitude).toBeCloseTo(1);
    });
