 ...
 */
export class EmailDetector implements PIIDetector {
   ...
   */
  private readonly emailRegex = /\b[A-Za-z0-9][A-Za-z0-9._%+-]*@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g;
  ...
  private validateEmail(email: string): number {
    ...
    if (localPart.length === 0 || localPart.length > 64) return 0;
    if (localPart.startsWith('.') || localPart.endsWith('.')) return 0;
    if (localPart.includes('..')) return 0;

    // Domain part validation
    if (domainPart.length === 0 || domainPart.length > 253) return 0;
    if (domainPart.startsWith('.') || domainPart.endsWith('.')) return 0;
    if (domainPart.includes('..')) return 0;

    // Check for valid TLD
    const domainLabels = domainPart.split('.');
    if (domainLabels.length < 2) return 0; // Must have at least domain.tld

    const tld = domainLabels[domainLabels.length - 1];
    if (tld.length < 2) return 0;

    // Check each domain label
    for (const label of domainLabels) {
      if (label.length === 0) return 0;
      if (label.startsWith('-') || label.endsWith('-')) return 0;
    }

    // Increase confidence for common patterns
    if (/^[a-zA-Z0-9._-]+$/.test(localPart)) confidence += 0.1;
    if (domainLabels.length >= 2) confidence += 0.1;
    if (/^[a-zA-Z]+$/.test(tld)) confidence += 0.1;
 ...
 */
export function createEmailDetector(): EmailDetector {
