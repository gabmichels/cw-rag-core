 ...
 */
export class NationalIdDetector implements PIIDetector {
  public readonly type = 'national_id' as const;
  ...
  private validateSSN(ssn: string): boolean {
    ...
    if (!ssnPattern.test(ssn)) return false;
    ...
    if (cleanSSN.slice(3, 5) === '00') return false;
    if (cleanSSN.slice(5) === '0000') return false;
    if (cleanSSN.startsWith('666')) return false;
    if (cleanSSN.startsWith('9')) return false;
   ...
   */
  private validateUKNI(ni: string): boolean {
    const niPattern = /^[A-Z]{2}\s?\d{2}\s?\d{2}\s?\d{2}\s?[A-Z]$/;
    if (!niPattern.test(ni)) return false;
   ...
   */
  private validateCanadianSIN(sin: string): boolean {
    const sinPattern = /^\d{3}[-\s]?\d{3}[-\s]?\d{3}$/;
    if (!sinPattern.test(sin)) return false;
 ...
 */
export function createNationalIdDetector(): NationalIdDetector {
