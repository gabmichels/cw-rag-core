...
export class APIKeyDetector {
    ...
    validateAPIKey(key) {
        ...
        if (/^(.)\1+$/.test(key))
            confidence -= 0.2; // All same character
        if (/^(abc|123|test|demo)/i.test(key))
            confidence -= 0.1; // Common test patterns (removed 'example')
...
export class JWTDetector {
    type = 'jwt_token';
    jwtPattern = /\beyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]*\b/g;
    detect(text, options = {}) {
 ...
 * Factory functions
 */
export function createAPIKeyDetector() {
    return new APIKeyDetector();
}
export function createAWSKeyDetector() {
    return new AWSKeyDetector();
}
export function createJWTDetector() {
