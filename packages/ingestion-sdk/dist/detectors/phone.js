...
export class PhoneDetector {
    ...
    validatePhone(phone) {
        ...
        if (digitsOnly.length >= 10)
            confidence += 0.2;
        if (phone.includes('+'))
            confidence += 0.1; // International format
        if (/\(\d{3}\)/.test(phone))
            confidence += 0.1; // US area code format
        if (/\d{3}-\d{3}-\d{4}/.test(phone))
            confidence += 0.1; // US standard format
        // Decrease confidence for suspicious patterns
        if (/^(\d)\1+$/.test(digitsOnly))
            confidence -= 0.3; // All same digits
        if (digitsOnly === '1234567890' || digitsOnly === '0123456789')
            confidence -= 0.4; // Sequential
 ...
 */
export function createPhoneDetector() {
